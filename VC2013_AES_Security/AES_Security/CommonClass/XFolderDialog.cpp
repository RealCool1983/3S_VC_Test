...
//static LPCTSTR szFilter = _T("*.bin|*.txt||");
CString strFilter = _T("BIN(*.bin)|*.bin|")
						  _T("TXT(*.txt)|*.txt ||");
...
: CFileDialog(TRUE, NULL, NULL, dwFlags | OFN_HIDEREADONLY, strFilter, pParentWnd)
//=============================================================================
{
...
//=============================================================================
{
	...
	if (pWndOkButton && ::IsWindow(pWndOkButton->m_hWnd))
	{
		...
		pWndOkButton->SetWindowText(_T("Open"));  // Add by Brad Kuo
	}
	...
	if (pWnd && ::IsWindow(pWnd->m_hWnd))
	{
		...
		pWnd->SetWindowText(_T("Cancel"));  // Add by Brad Kuo
	}
