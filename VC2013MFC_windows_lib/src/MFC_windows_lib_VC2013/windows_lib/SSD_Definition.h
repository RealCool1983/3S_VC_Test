...
enum
{
    ...
    RDT_RESULT_OTHER_FAIL    // 6, other faild
};
...
typedef struct _MP_USE_DATA_CONFIG_INFO
{
    ...
    BYTE  KDI_KeyPro_User_Data[512];  // save Kingston KeyPro user data that is encrypted by AES
    BYTE  KDI_XOR_Key[1024];          // save Kingston XOR key define in INI file
...
typedef union _RDT_BLOCK_RESULT
{
    struct
    {
        ...
        UINT16  Cycle;                       // 2 Byte
        UINT16  Max_tPro;                    // 2 Byte    (whole weru? ms?)
...
typedef struct _RDT_RESULT 
{
    union
    {
        struct
        {
            ...
            UINT32  stage;                        // preload data
            UINT32  pwr_cycle_cnt;
        } h;
        BYTE rev[128];
    } head;
